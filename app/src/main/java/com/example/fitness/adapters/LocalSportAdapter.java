...
import com.example.fitness.fragments.LocalSportFragment;
import com.example.fitness.models.LocalFitness;

import java.util.ArrayList;
import java.util.List;

public class LocalSportAdapter extends RecyclerView.Adapter<LocalSportAdapter.LocalSportViewHolder> implements Filterable {
    private String username;
    private boolean searching = false;

    public LocalSportAdapter(String username) {
        this.username = username;
    }

    private List<LocalFitness> localFitnesses = LocalFitnessFragment.database.localFitnessDao().get(username, "Sport");
    private List<LocalFitness> filteredLocalFitness = new ArrayList<>();
    private List<LocalFitness> filtered = new ArrayList<>();

    public Filter getFilter() {
        return new LocalSportFilter();
    }

    public class LocalSportFilter extends Filter {
    ...
    public class LocalSportViewHolder extends RecyclerView.ViewHolder {
        private TextView localFitnessName;
        private LinearLayout localFitnessRow;
        public LocalSportViewHolder(@NonNull final View itemView) {
            super(itemView);
    ...
    @NonNull
    @Override
    public LocalSportViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.home_row, parent, false);
        return new LocalSportViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull LocalSportViewHolder holder, int position) {
        LocalFitness current;
        if (searching) {
            current = filtered.get(position);
        } else {
            localFitnesses = LocalFitnessFragment.database.localFitnessDao().get(username, "Sport");
