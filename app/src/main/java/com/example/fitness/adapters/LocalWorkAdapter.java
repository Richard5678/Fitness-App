...
import com.example.fitness.fragments.LocalWorkFragment;
import com.example.fitness.models.LocalFitness;

import java.util.ArrayList;
import java.util.List;

public class LocalWorkAdapter extends RecyclerView.Adapter<LocalWorkAdapter.LocalWorkViewHolder> implements Filterable {
    private String username;
    private boolean searching = false;

    public LocalWorkAdapter(String username) {
        this.username = username;
    }

    private List<LocalFitness> localFitnesses = LocalFitnessFragment.database.localFitnessDao().get(username, "Work");
    private List<LocalFitness> filteredLocalFitness = new ArrayList<>();
    private List<LocalFitness> filtered = new ArrayList<>();

    public Filter getFilter() {
        return new LocalWorkFilter();
    }
    
    public class LocalWorkFilter extends Filter {
    ...
    public class LocalWorkViewHolder extends RecyclerView.ViewHolder {
        private TextView localFitnessName;
        private LinearLayout localFitnessRow;
        public LocalWorkViewHolder(@NonNull final View itemView) {
            super(itemView);
    ...
    @NonNull
    @Override
    public LocalWorkViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.home_row, parent, false);
        return new LocalWorkViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull LocalWorkViewHolder holder, int position) {
        LocalFitness current;
        if (searching) {
            current = filtered.get(position);
        } else {
            localFitnesses = LocalFitnessFragment.database.localFitnessDao().get(username, "Work");
